...
noise_covariance_slider = Slider(title="Шум (дисперсія)", value=INIT_NOISE_COVARIANCE, start=0.01, end=1.0, step=0.01, **noise_slider_styles)
gaussian_window_slider = Slider(title="Gaussian Window", value=5, start=3, end=21, step=2, **changing_noise_slider_styles)
gaussian_std_slider = Slider(title="Gaussian STD", value=2, start=0.1, end=5.0, step=0.1, **changing_noise_slider_styles)
...
def export_data(filename='data_export_bokeh.csv'):
    time, y = harmonic_with_noise(amp_slider.value, freq_slider.value, phase_slider.value,
                                  noise_mean_slider.value, noise_covariance_slider.value,
    ...
    data = zip(time, y, filtered_y, [amplitude] * len(time), [frequency] * len(time),
               [phase] * len(time), [noise_mean] * len(time), [noise_covariance] * len(time),
...
def reset():
    ...
    noise_covariance_slider.value = INIT_NOISE_COVARIANCE
...
layout = column(
    ...
    row(noise_mean_slider, noise_covariance_slider),
